...
| 1   | Scripts Without Reflashing                | Blink, SOS and a press counter, swapped over the air  | Sandbox workflow, task model, persistent scripts        | None                         |
| 2   | Dim an LED from a Slider                  | A dashboard slider driving PWM brightness             | Register indices, control widgets, retained events      | LED and resistor             |
| 3   | A Night Light That Thinks for Itself      | Light sensor, threshold and LED, decided on the device | Edge logic, publishing only changes, works offline     | Potentiometer or LDR         |
