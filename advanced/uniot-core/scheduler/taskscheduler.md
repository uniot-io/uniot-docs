...
* **`TaskScheduler &push(ISchedulerConnectionKit &connection)`**\
  Enables the scheduler to add tasks via the [`ISchedulerConnectionKit`](../../../uniot-core/scheduler/ischedulerconnectionkit.md) interface, providing flexibility in task sources. This allows external components to integrate their tasks seamlessly into the scheduler.
* **`void loop()`**\
